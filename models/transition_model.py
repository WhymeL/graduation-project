...
class TransitionModel:
    def __init__(self,
        ...
        self.rew_normalizer = StandardNormalizer()
    ...
    def eval_data(self, data, update_elite_models=False):
        ...
        # print(torch.isnan(obs_list).any())
        # print(torch.isnan(action_list).any())
        # print(torch.isnan(next_obs_list).any())
        # print(torch.isnan(reward_list).any())
        delta_obs_list = next_obs_list - obs_list
        # obs_list, action_list = self.transform_obs_action(obs_list, action_list)
        model_input = torch.cat([obs_list, action_list], dim=-1).float().to(util.device)
    ...
    def update(self, data_batch):
        ...
        # print(torch.isnan(obs_batch).any())
        # print(torch.isnan(action_batch).any())
        # print(torch.isnan(next_obs_batch).any())
        # print(torch.isnan(reward_batch).any())

        delta_obs_batch = next_obs_batch - obs_batch
        # obs_batch, action_batch = self.transform_obs_action(obs_batch, action_batch)

        # predict with model
        model_input = torch.cat([obs_batch, action_batch], dim=-1).float().to(util.device)
    ...
    def predict(self, obs, act, deterministic=False):
        ...
        model_input = torch.cat([obs, act], dim=-1).to(util.device)
