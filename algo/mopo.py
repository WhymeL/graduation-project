...
class MOPO:
    ...
    def learn_dynamics(self):
        ...
        eval_mse_losses_list.append(eval_mse_losses.mean())
        ...
        while not break_training:
            for train_data_batch in dict_batch_generator(train_data, self.model_batch_size):
                ...
                train_mse_losses_list.append(model_log_infos["loss/train_model_loss_mse"])
                model_train_iters += 1
        ...
        plt.subplot(1, 2, 1)
        plt.plot(train_mse_losses_list)
        plt.xlabel('step')
        plt.ylabel('mse_loss')
        plt.title('train mse losses')
        plt.subplot(1, 2, 2)
        plt.plot(eval_mse_losses_list)
        plt.xlabel('epoch')
        plt.ylabel('mse_loss')
        plt.title('eval mse losses')
        plt.show()
